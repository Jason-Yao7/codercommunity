...
public class VideoService {
    ...
    //存在两次数据的添加，使用@Transactional注解，使用事务进行管理，防止第一次添加成功后，第二次添加失败，事务回滚
    @Transactional
...
//        System.out.println("This is " + similarity.userSimilarity(18, 20));
        //获取用户邻居
        ...
        List<Video> relist = new ArrayList<>();
        if (itemIds.size() == 0) {
            return relist;
        } else {
    ...
    public List<VideoBinaryPicture> convertVideoToImage(Long videoId, String fileMd5) throws Exception {
        ...
        String filePath = "/Users/ysc/devTools/filetest/tmpfile/fileForVideoId" + videoId + "." + file.getType();
